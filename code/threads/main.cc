...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        Userprogs.push_back(userProgName);
	    i++;
	}
    ...
    int progcount = Userprogs.size();
    for(int i =0; i<progcount; i++){

        Thread *newprog = new Thread("New user program");
        newprog->Fork((VoidFunctionPtr)RunUserProg, (void *) Userprogs.front());
        Userprogs.pop_front();
    }
